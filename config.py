...
# AI APIs
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "")   # دیگر استفاده نمی‌شود، اما می‌توان نگه داشت
GROQ_API_KEY = os.getenv("GROQ_API_KEY", "")       # جدید
...
def validate_config():
    ...
    if not GROQ_API_KEY:
        errors.append("GROQ_API_KEY not set (required for AI responses)")
    if errors:
        for err in errors:
            print(f"❌ {err}")
        return False

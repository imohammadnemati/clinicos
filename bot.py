...
# ========== Helper Functions ==========
def get_patient_by_telegram_id(
    telegram_id: int, db, clinic_id: Optional[int] = None
) -> Optional[Patient]:
    """Return only the patient belonging to the trusted clinic context."""
    if clinic_id is None:
        clinic_id = get_user_clinic_id(telegram_id)
    if clinic_id is None:
        return None
    ...
    return (
        ...
        .filter(
            ...
            PatientAlias.external_user_id == str(telegram_id),
...
def get_or_create_patient_by_telegram(
    telegram_id: int, name: str, db, clinic_id: Optional[int] = None
) -> Patient:
    """Create a patient only when a trusted tenant is available."""
    if clinic_id is None:
        clinic_id = get_user_clinic_id(telegram_id)
    if clinic_id is None:
        raise ValueError("Trusted clinic context is required to create a patient.")

    patient = get_patient_by_telegram_id(telegram_id, db, clinic_id=clinic_id)
    if patient:
        return patient
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Always show language selection menu first."""
...
async def appointment_service_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(get_text("appt_enter_date", lang))
    return APPT_DATE

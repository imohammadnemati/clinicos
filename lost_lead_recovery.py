...
from models import Lead, PatientAlias, Patient, FollowupWindow, Clinic
...
async def send_recovery_message(
    ...
    try:
        ...
        if not alias or not alias.external_user_id:
            print(f"شماره تلگرام برای بیمار {patient_id} یافت نشد.")
            return False
        ...
        print(f"خطا در ارسال پیام بازیابی: {e}")
        return False
    finally:
        db.close()
...
async def recover_lost_leads():
    ...
    """
    print("🔄 شروع بازیابی لیدهای از دست رفته...")
    db = SessionLocal()
    now = datetime.utcnow()
    ...
    for lead in leads:
        ...
        if success:
            lead.recovery_attempts += 1
            recovered_count += 1
            print(f"✅ پیگیری لید {lead.id} - تلاش {lead.recovery_attempts}")
            
            # اگر به حداکثر تلاش رسید، وضعیت لید به lost تغییر کند
            if lead.recovery_attempts >= MAX_RECOVERY_ATTEMPTS:
                lead.pipeline_stage = 'lost'
                print(f"⚠️ لید {lead.id} پس از {MAX_RECOVERY_ATTEMPTS} تلاش بی‌نتیجه به lost تغییر یافت.")
    ...
    db.close()
    print(f"✅ بازیابی لیدها: {recovered_count} پیام ارسال شد، {skipped_count} رد شد.")
